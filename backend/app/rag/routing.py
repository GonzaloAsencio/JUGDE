...
def is_hard_query(*, card_count: int, keyword_count: int) -> bool:
    ...
    OR keywords >= 2 routes all 4 residual misses plus 10 more hard/medium
    questions and zero easy ones. Question length was evaluated as a third
    signal and rejected (no additional target coverage, more over-routing).
    """
    return card_count >= 2 or keyword_count >= 2
...
@lru_cache(maxsize=1)
def _load_card_sections() -> tuple[tuple[str, str], ...]:
    """cards.md split by '## <card name>' headers, as a hashable tuple of
    (name, section text) pairs so the lru_cache stays immutable."""
...
def build_stuffed_chunks(question: str, *, known_keywords: frozenset[str]) -> list[Chunk] | None:
