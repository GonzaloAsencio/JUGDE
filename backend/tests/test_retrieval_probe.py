...
# (4 of the 5 questions route and answer correctly). routing_decision IS
# routing.should_route — the same object production calls, see the identity
# test below.
# ---------------------------------------------------------------------------
...
def test_routing_decision_false_below_threshold():
    ...
    # recall is a real production signal.
...
def test_routing_decision_forwards_every_is_hard_query_knob():
    ...
    classifier grows a knob the gate does not forward. (It caught exactly that
    on 2026-07-17: `relaxed` arrived on is_hard_query with a dead lever and the
    probes' mirror never followed. Both the lever and the knob are gone now, but
...
def test_routing_decision_is_productions_gate_by_construction():
    """The other half of the same debt, closed by extraction instead of mirror.

    routing_decision used to MIRROR the gate (same flag, same classifier, copied
    by hand) — and a mirror is exactly what drifted when the classifier grew
    `relaxed`. Now routing.py owns should_route, pipeline.answer_question calls
    it, and routing_decision IS it: identity, not equality. There is no second
    copy left to drift.
    """
    from app.rag import pipeline
    from app.rag.routing import should_route

    assert routing_decision is should_route, (
        "routing_decision must BE routing.should_route — a wrapper or copy "
        "reopens the drift the extraction closed"
    )
    assert pipeline.should_route is should_route, (
        "pipeline must route through the same should_route object the probes use"
    )
