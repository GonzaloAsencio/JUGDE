...
# ---------------------------------------------------------------------------
# Frontend lookup index — name → image_url mapping for card preview popovers
# ---------------------------------------------------------------------------
...
def build_card_index(cards: list[dict]) -> list[dict]:
...
def serialize_card_index_ts(entries: list[dict]) -> str:
    """Render the index as a TypeScript module with a typed `as const` array."""
    body = json.dumps(entries, indent=2, ensure_ascii=False)
    return (
        f"{_TS_BANNER}"
        f"{_TS_INTERFACE}\n"
        f"export const CARD_INDEX: readonly CardIndexEntry[] = {body} as const;\n"
    )
...
if __name__ == "__main__":
    ...
    ts_out = Path("../frontend/lib/cardIndex.ts")
    ...
    ts_out.write_text(serialize_card_index_ts(index), encoding="utf-8")
    print(f"Generated: {ts_out} ({len(index)} cards in lookup index)")
