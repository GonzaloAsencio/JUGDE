...
<!-- TODO: add screenshot of the interface -->

**[Live Demo](#)** | **[Blog Post](docs/blog/post.md)** | **[Video](#)**

---

## What It Does

Riftbound Judge AI answers rules questions about the Riftbound trading card game by retrieving relevant passages from the official rulebook and using Gemini 2.0 Flash to generate a grounded, cited answer. The system refuses to speculate — if the retrieved context does not contain the answer, the system defers rather than fabricates.
...
| Technology | Role | Why |
|---|---|---|
| Python / FastAPI | API server | Async, fast, Pydantic validation built-in |
...
| Gemini 2.0 Flash | LLM generation | Free tier (1M tok/day), fast, good instruction following |
| Upstash Redis | Response cache | Serverless Redis — optional, disabled if env vars absent |
| Langfuse | LLM tracing | Traces retrieval + generation per request |
| Sentry | Error reporting | Optional — disabled if DSN not set |
| Next.js 15 | Frontend | App Router, Vercel deployment |
| slowapi | Rate limiting | Per-IP rate limits on the API |
...
**Methodology**: Eval set size TBD. Evaluation framework: RAGAS. Latency measured server-side (excludes network). Cost computed from Gemini 2.0 Flash token pricing. Each configuration: mean of 3 independent runs. Hardware: TBD.

---

## Key Decisions

The architecture reflects several explicit tradeoffs documented as Architecture Decision Records. ADRs are the single source of truth for why things are the way they are — this section links to them; it does not restate their content.

The most consequential decisions were: choosing a local embedding model over a hosted API to eliminate per-query cost; keeping everything in a single Postgres instance to avoid dual-service complexity; and switching from vector-only to hybrid retrieval after observing exact-token query failures in manual testing. Entity resolution was explicitly deferred — there was no eval data to justify the complexity at v1.
...
cp .env.example .env
# Edit .env and set: DATABASE_URL, GEMINI_API_KEY
# Optional: UPSTASH_REDIS_URL, UPSTASH_REDIS_TOKEN, LANGFUSE_*, SENTRY_DSN

# Ingest the corpus (runs bge-m3 locally — first run downloads the model, ~1.2 GB)
...
# Edit .env.local and set: FASTAPI_URL=http://localhost:8000
...
# TODO: eval runner script pending (see FUTURE_WORK.md)
```
...
The evaluation set was built manually, not generated by an LLM. Each question was written by a human, cross-referenced against the rulebook, and assigned a reference answer and the expected source section(s). LLM-generated questions were rejected because they tend to mirror document structure — a question generated from a paragraph about combat timing will be phrased similarly to that paragraph, which artificially inflates retrieval metrics.
